...
function addInstant() {
	...
	var i, ww, hh, ff, yo, xo, tcolor, nocorner, historical, preserve, thinBorder, noBorder;	
	for(i=0;i<theimages.length;i++) {	
		...
		if(canvas.getContext && image.width>=64 && image.height>=64) {
			...
			ishadow = getClassValue(classes,"ishadow");
			if(ishadow>0) opacity=ishadow/100;
			thinBorder = getClassAttribute(classes,'thinBorder');
			noBorder = getClassAttribute(classes,'noBorder');
			icolor = getClassColor(classes,"icolor");
			if(icolor!=0) color = icolor;
			itiltleft = getClassAttribute(classes,"itiltleft");
			itiltright = getClassAttribute(classes,"itiltright");
			...
			if(thinBorder == true) border = Math.round(((canvas.width+canvas.height)/2)*0.02);
			if(noBorder == true){
			border = 0;
			inset = 0;
			offset = 0;
